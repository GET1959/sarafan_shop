...
from shop.serializers import ProductSerializer, ShopCartSerializer
...
class Cart:
    ...
    def __iter__(self):
        ...
        """
        product_ids = self.cart.keys()
        products = CartItem.objects.filter(product_id__in=product_ids)
        cart = self.cart.copy()
        for product in products:
            cart[str(product.id)]["product"] = ShopCartSerializer(product).data
        for item in cart.values():
            item["price"] = Decimal(item["price"])
            item["total_price"] = item["price"] * item["quantity"]
            yield item
